    ...
    for (const entry of monitorList) {
      ...
      if (result.status === 'banned' && !entry.lastStatusWasBanned) {
        ...
        const embed = new EmbedBuilder()
          ...
          .setImage('https://i.imgur.com/TpZXYCG.jpeg')
          .setFooter({
      ...
      // ====== USER GOT UNBANNED ======
        ...
        const embed = new EmbedBuilder()
          ...
          .addFields(
            ...
            { name: 'Profile URL', value: `instagram.com/${entry.username}`, inline: false },
            { name: 'Reason', value: entry.reason || 'Ban lifted', inline: false },
            { name: 'Time Taken', value: timeDisplay, inline: false },
        ...
        if (channel) {
          await channel.send({
            content: `The Account "@${entry.username}" Unbanned Successfully`,
            embeds: [embed]
          });
        }
...
**Features:**
 Ban detection → Notification in #bans
 Unban detection → Notification in #bans
